...
DSC_MODEL_PAYLOAD = {
    ...
    "projectId": "ocid1.datascienceproject.oc1.iad.xxx",
    ...
    "artifact": "ocid1.datasciencemodel.oc1.iad.xxx.zip",
}
...
class TestGenericModel:
    ...
    def test_deploy_success(self, mock_model_deployment_state):
        ...
        with patch.object(ModelDeployer, "deploy") as mock_deploy:
            expected_result = ModelDeployment()
            mock_deploy.return_value = expected_result

            for session in input_params:
                assert self.generic_model.deploy(**session) == expected_result

                session_copy = copy(session)
                session_copy["project_id"] = session.get("project_id", PROJECT_OCID)
                session_copy["compartment_id"] = session.get(
                    "compartment_id", _COMPARTMENT_OCID
                )
                if session.get("deployment_log_group_id"):
                    session_copy["access_log_group_id"] = session.get(
                        "deployment_log_group_id"
                    )
                    session_copy["predict_log_group_id"] = session.get(
                        "deployment_log_group_id"
                    )

                    session_copy["access_log_id"] = session.get(
                        "deployment_access_log_id"
                    )
                    session_copy["predict_log_id"] = session.get(
                        "deployment_predict_log_id"
                    )

                session_copy["instance_shape"] = session.get(
                    "deployment_instance_shape"
                )
                session_copy["instance_count"] = session.get(
                    "deployment_instance_count"
                )
                session_copy["bandwidth_mbps"] = session.get(
                    "deployment_bandwidth_mbps"
                )
                session_copy["memory_in_gbs"] = session.get("deployment_memory_in_gbs")
                session_copy["ocpus"] = session.get("deployment_ocpus")

                expected_props = ModelDeploymentProperties(
                    model_id=test_model_id, **session_copy
                )

                mock_deploy.assert_called_with(
                    properties=expected_props,
                    wait_for_completion=session.pop("wait_for_completion", False),
                    max_wait_time=session.pop("max_wait_time", DEFAULT_WAIT_TIME),
                    poll_interval=session.pop("poll_interval", DEFAULT_POLL_INTERVAL),
                )

    @patch("ads.common.auth.default_signer")
    @patch("ads.common.oci_client.OCIClientFactory")
    def test_deploy_with_default_display_name(self, mock_client, mock_signer):
    ...
    @patch("ads.common.auth.default_signer")
    @patch("ads.common.oci_client.OCIClientFactory")
    def test_update_deployment_instance_level_without_id(
        ...
        with pytest.raises(
            ...
            generic_model.update_deployment(
                properties=None,
                wait_for_completion=True,
                max_wait_time=100,
                poll_interval=200,
    ...
    @patch("ads.common.auth.default_signer")
    @patch("ads.common.oci_client.OCIClientFactory")
    def test_restart_deployment_fail(
        self, mock_client, mock_signer, mock_deactivate, mock_activate
    ):
        generic_model = GenericModel(estimator=TestEstimator())
        with pytest.raises(
